...
# -*- coding: utf-8 -*-
"""
Python implementation of mostDominantTranscriptSwitch.pl (fixed, v3)
- GTEx enrichment uses minExp*0.1 (Perl'le uyum)
- Sign-test full precision (print 3dp)
- Missed-interaction sayaçları: yoksa -1/-1
"""
...
def read_expression_file(filename, enst2ensg, redundant, verbose=False):
    ...
    # relative expressions (precise floats + print strings at 3dp)
    rel_enst_express = {}
    ...
    # 3dp stringleri yalnız yazım için üretelim
...
def read_isoform_int_file(filename, min_string_score):
    ...
    Always compute int_n (total qualified STRING interactions).
    """
    miss_int4isoform = {}
    int_n = {}
    with gzip.open(filename, 'rt') as f:
        for line in f:
            ...
            # existing interactions (only count total STRING int)
            if exist_int:
...
def main():
    parser = argparse.ArgumentParser(description='Dominant Transcript Switch Analysis (fixed v3)')
    ...
    enrichment_pcawg, all_enrichment_pcawg = enrichment(
        enst_express_pcawg, args.minExp, args.minEnr, args.pcawgEnrichmentFile
    )

    if args.verbose:
        print("Calculating enrichment GTEx...", file=sys.stderr)
    # IMPORTANT: Perl davranışı — GTEx için eşik minExp * 0.1
    enrichment_gtex, _ = enrichment(
        enst_express_gtex, args.minExp * 0.1, args.minEnr, args.gtexEnrichmentFile
    )

    # header
    print("#ENSG\tENSPcanon\tcMDT\tRNAseqAliquotID\tGTExMDT\tTotalRelevantGTExSamples\t"
          "GTExMDTmedianEnrichment\tPvalue\tQvalue\tNumberOfGTExMDT\tcMDTenrichment\t"
          "GTExMDTmedianRelExpression\tcMDTrelExpression\tcMDTuniqMissedInt\tTotalNumberOfSTRINGint\t"
          "cMDTnumberOfUniqMissedInt\tNumberOfCommonMissedInt")

    pvalue_file = open(args.pvalueFile, 'w') if args.pvalueFile else None
    if pvalue_file:
        pvalue_file.write("#ENSG\tcMDT\ttRNAseqAliquotID\tMedianRelMDTexpressionInNormal\t"
                          "RelMDTexpressionInCancer\tPCAWGhigherExpressionN\tPCAWGlowerExpressionN\t"
                          "Pvalue\tRelMDTexpressionInGTEx\n")

    random_pvalue_file = open(args.randomPvalueFile, 'w') if args.randomPvalueFile else None
    if random_pvalue_file:
        random_pvalue_file.write("#ENSG\tDomCancerTrans\tCancerSampleId\tMedianRelMDTexpressionInNormal\t"
                                 "RelMDTexpressionInCancer\tPCAWGhigherExpressionN\tPCAWGlowerExpressionN\tPvalue\n")

    # collect rows after p<maxQ (order preserved for FDR)
    records = []
    for ensg in sorted(enrichment_pcawg.keys()):
        ...
        for enst in sorted(enrichment_pcawg[ensg].keys()):
            ...
            for sample, enrichment_pcawg_val in sorted(enrichment_pcawg[ensg][enst].items()):
                ...
                # Only analyze if ENST is NOT MDT in GTEx, but gene has MDT(s)
                ...
                if pvalue_file:
                    pvalue_file.write(f"{ensg}\t{enst}\t{sample}\t{rel_expression_gtex_median:.3f}\t"
                                      f"{enst_rel_express_pcawg:.3f}\t{pos}\t{neg}\t{pvalue:.3e}\t{gtex_vals_print}\n")
                ...
                # Skip if no GTEx MDT covers >=50% of relevant samples
                skip = True
    ...
    # FDR (records already p-filtered; order preserved)
    if records:
