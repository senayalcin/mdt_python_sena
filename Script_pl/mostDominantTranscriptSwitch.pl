...
&GetOptions(
    ...
    "redundantFile:s"        => \$redundantFile,       # e.g. redundantENST.txt
...
sub readEnst2ensgFile {
    my %enst2ensg = (); 
    open(F1, "gzcat $enst2ensgFile|") or die "\nERROR: Failed to open $enst2ensgFile: $!\n";
    while(my $l = <F1>) {
	next if($l !~ /^ENS/);
	chomp($l);
	my @a = split(/\t/, $l);
...
sub readExpressionFile {
    ...
    my $redundant = &readRedundantFile();

    my %enstExpress = ();
    my %ensgExpress = ();
    open(F2, "gzcat $file |") or die "\nERROR: Failed to open $file: $!\n";
    my $n = 0;
    my @samples = ();
    while(my $l = <F2>) {
	chomp($l);
	...
	if(exists $redundant->{$enst}) {
	    print "Replacing redundant cDNA $enst with $redundant->{$enst}\n" if($verbose);
	    $enst = $redundant->{$enst};
	}
	next if($enst !~ /ENST0/);
	$enst =~ s/\..*//;
	...
	for(my $i = $enstColumnNo+1; $i < @a; $i++) {
	    ...
	    if(exists $enstExpress{$ensg}->{$samples[$i]}->{$enst}) {
...
sub readCanonFile {
    my %canon = (); 
    open(F3, "gzcat $canonFile|") or die "\nERROR: Failed to open $canonFile: $!\n";
    while(my $l = <F3>) {
	next if($l !~ /^ENS/);
	chomp($l);
...
sub readIsoformIntFile {
    my %missInt4isoform = ();
    my %intN = ();
    open(F4, "gzcat $isoformIntFile |") or die "\nERROR: Failed to open $isoformIntFile: $!\n\n";
    while(my $l = <F4>) {
	next if($l !~ /^EN/);
	chomp($l);
...
###############################################################################
sub readRedundantFile {
    my %red = ();
    open(F10, $redundantFile) or die "\nERROR: Failed to open $redundantFile: $!\n\n";
    while(my $l = <F10>) {
	next if($l !~ /^EN/);
	chomp($l);
	my ($ensg, $enstSel, $enstRed) = split(/\t/, $l);
	$red{$enstRed} = $enstSel;
    }
    close(F10);
    print STDERR "Found ".(keys %red)." redundant ENSTs\n" if(defined $verbose);
    return \%red;
}
###############################################################################
sub readSequenceFile {
    my %sequences = ();
    open(F5, "gzcat $sequenceFile |") or die "\nERROR: Failed to open $sequenceFile: $!\n\n";
    while(my $l = <F5>) {
	next if($l !~ /^EN/);
	chomp($l);
...
my ($ensgExpressPcawg, $enstExpressPcawg, $relEnstExpressPcawg, $relEnstExpressStringPcawg, $allRelEnstExpressPcawg) = &readExpressionFile($expressPcawgFile, $sequences);
print STDERR "done\n" if($verbose);

print STDERR "Reading in $expressGtexFile ... " if($verbose);
my ($ensgExpressGtex,  $enstExpressGtex, $relEnstExpressGtex, $relEnstExpressStringGtex, $allRelEnstExpressGtex)  = &readExpressionFile($expressGtexFile, $sequences);
print STDERR "done\n" if($verbose);
...
#print STDERR "#2\n";
		foreach my $sample (sort keys %{$enrichmentPcawg->{$ensg}->{$enst}}) {
			...
			$R->send(qq`test = binom.test(c($pos, $neg), p=0.5, alternative="two.sided")`);
			...
			for(my $j= 0; $j<100; $j++) {
			    ...
			    $R->send(qq`rTest = binom.test(c($rPos, $rNeg), p=0.5, alternative="two.sided")`);
...
#print STDERR "#1 $pos\t$neg\n";
#print STDERR "#2 $pvalue\n";
			# So P-value must be smaller than maxQvalue
			if($pvalue < $maxQvalue) {
			    $pvalues .= $pvalue." ";

#print STDERR "#3 $pvalue\n";
